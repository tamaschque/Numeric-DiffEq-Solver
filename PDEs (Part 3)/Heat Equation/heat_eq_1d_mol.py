...
from numeric_de_solver.interactive_ivp_solver import matrix_rk4_step
...
class TempGrid:
    ...
    def plot(self):
        dx = WIDTH / (len(self.x_values) - 1)

        coords = []
        for i, temp in enumerate(self.temp):
            x = i * dx
            y = float(np.interp(temp, [T_MIN, T_MAX], [0, HEIGHT]))

            coords.extend([x,y])

        self.plot_line.delete()
        self.plot_line = pyg.shapes.MultiLine(*coords, thickness=5)
...
class HeatEqSim(pyg.window.Window):
    def __init__(self, *args, **kwargs):
        ...
        # init_temp = np.zeros_like(x_values)

        self.A = scipy.sparse.diags([1,-2,1], [-1,0,1], (NX,NX), dtype=float)
        self.A *= HEAT_DISSIPATION_FAC / DX**2
    ...
    def update(self, dt):
        # Simulation
        for _ in range(20):
            self.grid.temp = matrix_rk4_step(
                self.A, self.grid.temp, 1e-4
            )
...
if __name__ == "__main__":
    """
    See https://en.wikipedia.org/wiki/Method_of_lines
    """
    sim = HeatEqSim(
        width=WIDTH, height=HEIGHT,
        caption="Heat Equation Simulation"
    )
    pyg.app.run()