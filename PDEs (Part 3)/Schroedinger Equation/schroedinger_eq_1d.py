...
class WaveFunctionGrid:
    ...
    def normalize_coords(self, coords):
        ...
        ys = (1.0 + 1.0) * (ys - Y_MIN)/(Y_MAX - Y_MIN) - 1.0
    ...
    def get_trap_coords(self):
        trap_verts = []
        for i, (psi, psi_next) in enumerate(zip(self.absval()[:-1], self.absval()[1:])):

            x1 = np.interp(i, [0, len(self.psi)-1], [-1.02, 1.02])
            y1 = normalize(psi, Y_MIN, Y_MAX)

            x2 = np.interp(i+1, [0, len(self.psi)-1], [-1.02, 1.02])
            y2 = normalize(psi_next, Y_MIN, Y_MAX)
...
class SchroedingerSim(pyg.window.Window):
    def __init__(self, *args, **kwargs):
        ...
        psi0 = self._scale * gaussian_wavepacket(xs, self.x0, self.sigma, self.k0)

        r = EVOLUTION_FAC * DT / (2 * DX**2)

        A = scipy.sparse.diags([-r,1+2*r,-r], [-1,0,1], (NX,NX), dtype=complex)
        self.A = scipy.sparse.linalg.splu(A.tocsc())
        
        self.B = scipy.sparse.diags([r,1-2*r,r], [-1,0,1], (NX,NX), dtype=complex)
    ...
    def update(self, dt):
        if self.simulation_running:
            # Simulation
            for _ in range(50):
                b = self.B.dot(self.grid.psi)

                self.grid.psi = self.A.solve(b)
